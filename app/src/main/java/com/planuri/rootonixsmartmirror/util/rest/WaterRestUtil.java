...
public class WaterRestUtil {
    static final String TAG = WaterRestUtil.class.getSimpleName();
    static WaterRestUtil INSTANCE;

    Gson gson;
    Retrofit mRetrofit;
    NewsRetroService mService;

    static public WaterRestUtil getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new WaterRestUtil();
        }
        return INSTANCE;
    };

    private WaterRestUtil() {
